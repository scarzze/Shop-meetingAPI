...
class AuthUtils:
    ...
    def verify_token(self, token):
        ...
        """
        if self.debug_mode:
            # In debug mode, return a mock user with ID as string for Profile Service
            return {
                'id': 'test_user_1',  # Use string ID for Profile Service compatibility
    ...
    def auth_required(self, f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if isinstance(user, dict) and 'id' in user:
                # Convert user ID to string for Profile Service which needs string IDs
